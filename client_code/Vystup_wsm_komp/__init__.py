...
"""
      ...
      self.rich_text_vstupni_data.content = (
        f"Chyba při zobrazování vstupních dat: {str(e)}"
      )
  ...
  def _zobraz_normalizaci(self, norm_vysledky, vazene_hodnoty):
    ...
    try:
      ...
      md += (
        "| Varianta / Krit. | " + " | ".join(norm_vysledky["nazvy_kriterii"]) + " |\n"
      )
      ...
      self.rich_text_normalizace.content = (
        f"Chyba při zobrazování normalizace: {str(e)}"
      )
...
"""
  ...
  def _vytvor_graf_vysledku(self, saw_vysledky):
    ...
    try:
      ...
      fig = {
        ...
        "layout": {
          ...
          "xaxis": {
            ...
            "tickangle": -45
            if len(varianty) > 4
            else 0,  # Natočení popisků pro lepší čitelnost
          },
          "yaxis": {
            "title": "Skóre",
      ...
      return {"data": [], "layout": {"title": "Chyba při vytváření grafu"}}
